...
class DynamicWebsiteSpider(Spider):
    ...
    def parse(self, response, **kwargs):
        ...
        next_page_url = response.xpath('//a[@class="paginationItem paginationNextLink"]/@href').get()
        absolute_next_page_url = response.urljoin(next_page_url)
        yield Request(url=absolute_next_page_url)

    def parse_job(self, response, **kwargs):
        url = response.url
        title = response.xpath('//h2[@class="pageTitle1 color1 detailJob"]/text()').get()
        description = response.xpath('//div[@class="jobDetailDescription"]').get()
        apply_link = response.xpath('//div[@class="tPad1 jobButtonSet"]/a/@href').get()
